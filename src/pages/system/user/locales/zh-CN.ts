export default {
  'app.common.err.record.exist': '记录已存在',
  'app.common.label.operation': '操作',
  'app.common.label.memo': '备注',
  'app.common.label.columns-display-settings': '显隐',
  'app.common.label.select-all': '全选',
  'app.common.label.select-reversely': '反选',
  'app.common.label.open-in-new-window': '点击新窗口打开链接',
  'app.common.label.yes': '是',
  'app.common.label.no': '否',
  'app.common.bale.more': '更多',
  'app.common.bale.is-open': '是否开启',
  'app.common.validation.not-empty': '必填项不能为空',

  'app.user': '用户管理locales\n' + '_mock.ts\n' + 'index.tsx\n' + 'model.ts\n' + 'service.ts',

  'app.user.edit-the-user': '编辑用户',
  'app.user.add-new-user': '新增用户',
  'app.user.add-user-role': '分配角色',

  'app.user.label.current-user': '当前用户',
  'app.user.label.username': '用户名',
  'app.user.label.pwd': '密码',
  'app.user.label.salt': 'salt',
  'app.user.label.age': '年龄',
  'app.user.label.avatar': '头像',
  'app.user.label.email': '邮箱',
};
