export default {
  'app.common.err.record.exist': '記錄已存在',
  'app.common.label.operation': '操作',
  'app.common.label.memo': '備註',
  'app.common.label.columns-display-settings': '顯隱',
  'app.common.label.select-all': '全選',
  'app.common.label.select-reversely': '反選',
  'app.common.label.open-in-new-window': '點擊新窗口打開鏈接',
  'app.common.label.yes': '是',
  'app.common.label.no': '否',
  'app.common.bale.more': '更多',
  'app.common.validation.not-empty': '必填項不能為空',

  'app.config':'系统配置 管理',

  'app.config.edit-the-config': '编辑系统配置 ',
  'app.config.add-new-config': '新增系统配置 ',

  'app.config.label.name': '名称',
  'app.config.label.value': '配置值',
  'app.config.label.type': '配置关键字',
  'app.config.label.tenantId': '租户',
  'app.config.label.delFlag': '删除标识（0-正常,1-删除）',
  'app.config.label.createTime': '创建时间',
  'app.config.label.updateTime': '更新时间',
  'app.config.label.creator': '创建人',
  'app.config.label.updater': '修改人',
};
